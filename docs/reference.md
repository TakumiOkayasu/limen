...
MacからWXR (192.168.100.1) にアクセスするには、VyOS経由のルートを追加する必要がある。

```bash
# [Mac] WXR管理画面にアクセスするためのルート追加 (再起動で消える)
sudo route add -net 192.168.100.0/24 192.168.1.1
```

追加後、ブラウザで `http://192.168.100.1` にアクセス可能。

...
**NIC構成**:
- 自作ルーター側: オンボード1GbE NIC (デバイス名は `show interfaces` で確認)
- WXR側: 1Gポート (10GポートはMAP-E WANで使用中)

**IP設計**:
| 機器 | インターフェース | IPアドレス | 役割 |
|------|------------------|------------|------|
| WXR | LAN側(1G) | 192.168.100.1/24 | IPv4 GW |
| 自作ルーター | オンボード1GbE (要確認) | 192.168.100.2/24 | IPv4転送元 |
...
### 4-3-2. 物理接続
1. 自作ルーターのオンボード1GbE NIC (デバイス名要確認) をWXRの1Gポートに接続
   - WXRの10Gポートは既にWAN(MAP-E)で使用中
   - オンボードNICを使用 (5GbE RTL8126はフレキシブル用途のため使用しない)
   - IPv4は1Gbps上限となるが、IPv6が主役なので実用上問題なし

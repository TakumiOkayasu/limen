...
2. **`secret/docs/` ディレクトリ内のドキュメントを確認する**
   - 特に `secret/docs/reference.md` (機器情報、MACアドレス一覧、環境情報)
   - **`secret/docs/hardware-specs.md`** (使用機材の詳細スペック、ポート数、対応速度)
   - 関連する phase ドキュメント
   - トラブルシューティング履歴
3. **過去の失敗パターンを確認する** (`secret/docs/troubleshooting-*.md`, `secret/docs/failure-log-*.md`)
4. **使用機材について質問する前に `secret/docs/hardware-specs.md` を確認する**
...
→ [secret/docs/phase0-install.md](secret/docs/phase0-install.md)
...
### Phase 1: LAN側SSH有効化
→ [secret/docs/phase1-ssh.md](secret/docs/phase1-ssh.md)

- [x] 1-1: SSH有効化（LAN側 192.168.1.1 のみ）
- [x] 1-2: SSH公開鍵登録（ed25519, パスワード認証無効）

### Phase 2: IPv6基盤構築
→ [secret/docs/phase2-ipv6.md](secret/docs/phase2-ipv6.md)

- [x] 2-1: RA受信・DHCPv6-PD取得 (2404:7a82:4d02:4100::/56)
  - DUID-LL形式必須: `00:03:00:01:MAC`
  - 詳細は [troubleshooting-dhcpv6-pd.md](secret/docs/troubleshooting-dhcpv6-pd.md)
...
→ [secret/docs/phase3-wireguard.md](secret/docs/phase3-wireguard.md)
...
→ [secret/docs/phase4-wxr-routing.md](secret/docs/phase4-wxr-routing.md)
...
### Phase 5: 運用設定
→ [secret/docs/phase5-operations.md](secret/docs/phase5-operations.md)
...
### Phase 6: バックアップ体制
→ [secret/docs/phase6-backup.md](secret/docs/phase6-backup.md)
...
## リファレンス
→ [secret/docs/reference.md](secret/docs/reference.md)
...
- [IPv6疎通問題](secret/docs/troubleshooting-ipv6.md) - チェックリスト、過去の失敗パターン、VyOS制約
- [DHCPv6-PD取得問題](secret/docs/troubleshooting-dhcpv6-pd.md) - DUID-LL形式が必須
- [RA設定消失](secret/docs/troubleshooting-ra-missing.md) - クライアントがIPv6取得できない場合の診断手順
- [カーネル更新失敗](secret/docs/failure-log-2026-01-07-kernel-update.md) - MODULE_SIG_FORCE問題

---

## 復旧用リソース

→ **[secret/docs/disaster-recovery.md](secret/docs/disaster-recovery.md)** - 災害復旧ガイド (詳細手順)

| ファイル | 用途 |
|----------|------|
| `secret/scripts/vyos-restore.env.example` | シークレット値テンプレート |
| `secret/scripts/generate-vyos-restore.sh` | vbashスクリプト生成ツール |
| `secret/scripts/recovery-vyos-config.sh` | 手動復元用の手順表示 |
| `secret/scripts/backup-vyos-config.txt` | 設定コマンド一覧 (参照用) |
